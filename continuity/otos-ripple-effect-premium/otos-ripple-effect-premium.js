(function () {
  ...
  const totalNoIntervention = document.getElementById('ripple-total-noi');
  const totalWithIntervention = document.getElementById('ripple-total-otos');
  const totalAvoided = document.getElementById('ripple-total-avoided');
  ...
  const cycleMultipliers = { 1: 1, 2: 1.9, 3: 2.7 };
  const interventionFactors = { off: 1, on: 0.52 };
  ...
  function getAdjustedRingTotal(view, ringIndex, cycle = currentCycle, intervention = currentIntervention) {
    const base = ringTotals[view][ringIndex] || 0;
    if (ringIndex === 6) return 0;
    return base * (cycleMultipliers[cycle] || 1) * (interventionFactors[intervention] || 1);
  }

  function ringShortBadge(view, ringIndex) {
    if (ringIndex === 0) {
      return `${fmtGBP(getAdjustedRingTotal(view, ringIndex))} / year`;
    }
    const t = getAdjustedRingTotal(view, ringIndex);
    if (ringIndex === 6) return 'Future cost breeder';
    if (ringIndex === 7) return `~${fmtGBP(t)} / year`;
    return fmtGBP(t);
  }
  ...
  function setTogglesPressed() {
    ...
    document.querySelectorAll('[data-cycle]').forEach((btn) => {
      btn.setAttribute('aria-pressed', String(Number(btn.dataset.cycle) === currentCycle));
    });
    document.querySelectorAll('[data-intervention]').forEach((btn) => {
      btn.setAttribute('aria-pressed', String(btn.dataset.intervention === currentIntervention));
    });
  }
  ...
  function animateRingCounters() {
    ...
    const targets = [0, 1, 2, 3, 4, 5, 6, 7].reduce((acc, ringIndex) => {
      acc[ringIndex] = getAdjustedRingTotal(currentView, ringIndex);
      return acc;
    }, {});
  ...
  function updateImpactTotals() {
    if (!totalNoIntervention || !totalWithIntervention || !totalAvoided) return;
    const rings = [0, 1, 2, 3, 4, 5, 7];
    const noIntervention = rings.reduce((sum, ringIndex) => sum + getAdjustedRingTotal(currentView, ringIndex, currentCycle, 'off'), 0);
    const withIntervention = rings.reduce((sum, ringIndex) => sum + getAdjustedRingTotal(currentView, ringIndex, currentCycle, 'on'), 0);
    totalNoIntervention.textContent = fmtGBP(noIntervention);
    totalWithIntervention.textContent = fmtGBP(withIntervention);
    totalAvoided.textContent = fmtGBP(Math.max(0, noIntervention - withIntervention));
  }
  ...
  function initToggles() {
    ...
    document.querySelectorAll('[data-cycle]').forEach((btn) => {
      btn.addEventListener('click', () => {
        currentCycle = Math.max(1, Math.min(3, Number(btn.dataset.cycle) || 1));
        lockedRingIndex = null;
        setTogglesPressed();
        renderSvg();
      });
    ...
    document.querySelectorAll('[data-intervention]').forEach((btn) => {
      btn.addEventListener('click', () => {
        currentIntervention = btn.dataset.intervention === 'on' ? 'on' : 'off';
        lockedRingIndex = null;
        setTogglesPressed();
        renderSvg();
      });
